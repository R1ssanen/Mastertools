                   ...
                   const shader_t& t_Shader)
{
    ...
    if (static_cast<unsigned>(max_x - min_x) * static_cast<unsigned>(max_y - min_y) == 0)
    {
        return; // zero area triangle
    }

    const float InverseDoubleArea{1.f /
                                  DoubleTriangleArea(a.m_Pos.x, a.m_Pos.y, b.m_Pos.x, b.m_Pos.y, c.m_Pos.x, c.m_Pos.y)};
    ...
    for (int y = min_y; y <= max_y; y++)
    {
        ...
        for (int x = min_x; x <= max_x; x++)
        {
            ...
            if (Barycoord.x >= 0.f && Barycoord.y >= 0.f && Barycoord.z >= 0.f)
            {
                ...
                t_Shader(t_Context, t_Texture, glm::vec4(x, y, z, w), glm::vec2(u, v), Light);
            }
