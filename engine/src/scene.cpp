...
{
        return glm::vec3(
            Array[0].get<core::f32>(), Array[1].get<core::f32>(), Array[2].get<core::f32>()
        );
    }
...
void Scene::Update(f64 DeltaTime) {
    ...
    for (emitter_t& Emitter : m.Emitters) {
        Emitter->Update(DeltaTime);
    }
}

Scene Scene::New(const std::string& Path)
{
    ...
    for (const auto& [Key, Element] : SceneData.items())
    {
        ...
        else if (Key == "camera")
        {
            ...
            f32 CameraFov = Element.contains("fov") ? Element["fov"].get<f32>() : glm::degrees(Camera.GetFov());
            f32 CameraNear = Element.contains("near") ? Element["near"].get<f32>() : Camera.GetNear();
            f32 CameraFar = Element.contains("far") ? Element["far"].get<f32>() : Camera.GetFar();
        ...
        else if (Key == "lights")
        {
            for (const auto& Light : Element["point"])
            {
                ...
                f32 LightIntensity = Light.contains("intensity") ? Light["intensity"].get<f32>() : 1.f;
        ...
        else if (Key == "objects")
        {
            for (const auto& Object : Element)
            {
                ...
                const b8 IsMipmapped = Object.contains("mipmapped") ? Object["mipmapped"].get<b8>() : true;
                const b8 IsDoublesided = Object.contains("doublesided") ? Object["doublesided"].get<b8>() : false;
                u8 Miplevels = Object.contains("miplevels") ? Object["miplevels"].get<u8>() : 4;
                f32 AnimationPlaybackSpeed = Object.contains("playback") ? Object["playback"].get<f32>() : 1.f;
                ...
                if (Object.contains("texture") || Object.contains("color")) {
                    ...
                    if (Object.contains("texture")) {
                        ...
                        const b8 IsTransparent = Object.contains("transparent") ? Object["transparent"].get<b8>() : false;

                        if (std::filesystem::path(TexturePath).extension() == ".gif") {
                            Texture = AnimatedTexture::New(TexturePath, AnimationPlaybackSpeed, IsTransparent, IsDoublesided);
                        }

                        else {
                            Texture = IsMipmapped ? MipmapTexture::New(TexturePath, Miplevels, IsTransparent, IsDoublesided) : 
                                                    ImageTexture::New(TexturePath, IsTransparent, IsDoublesided);
                        }
                    }
                        ...
                        Texture = ColorTexture::New(Path, ToUint32(Color), IsTransparent, IsDoublesided);
                    }
        ...
        else if (Key == "emitters")
        {
            for (const auto& Emitter : Element) {
                glm::vec3 EmitterPos = Emitter.contains("pos") ? JsonArrayToVec3(Emitter["pos"]) : glm::vec3(0.f);
                f32 EmitterRadius = Emitter.contains("radius") ? Emitter["radius"].get<f32>() : 0.25f;
                u64 EmitterParticleCap = Emitter.contains("particles") ? Emitter["particles"].get<u64>() : 10;

                const b8 IsTransparent = Emitter.contains("transparent") ? Emitter["transparent"].get<b8>() : false;
                texture_t Texture = AnimatedTexture::New(Emitter["texture"].get<std::string>(), 1.f, IsTransparent, false);
            
                mesh_t EmitterMesh = LoadAsset(Emitter["path"].get<std::string>(), Texture->GetID());

                if (Emitter["type"].get<std::string>() == "disk") {
                    Emitters.push_back(DiskEmitter::New(EmitterMesh, EmitterParticleCap, EmitterPos, EmitterRadius));
                }

                else {
                    Emitters.push_back(SphericalEmitter::New(EmitterMesh, EmitterParticleCap, EmitterPos, EmitterRadius));
                }
            }
        }
    ...
    return Scene(_M{.Objects = Objects, .Emitters = std::move(Emitters), .Lights = Lights, .Skybox = Skybox, .Camera = Camera, .Name = Name});
}
