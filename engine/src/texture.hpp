...
class Texture {
 public:
  ...
  Texture(const std::string& t_Name, bool t_IsTransparent, bool t_IsDoublesided)
      : m_Name{t_Name},
        m_IsTransparent{t_IsTransparent},
        m_IsDoublesided{t_IsDoublesided} {}
...
class MipmapTexture : public Texture {
 public:
  ...
  MipmapTexture(const std::vector<image_t>& t_Mipmap,
                uint8_t t_Miplevels,
                const std::string& t_Name,
                bool t_IsTransparent,
                bool t_IsDoublesided)
      : 
        Texture(t_Name, t_IsTransparent, t_IsDoublesided), m_Mipmap{t_Mipmap},
        m_Miplevels{t_Miplevels} {}
...
class ColorTexture : public Texture {
 public:
  ...
  ColorTexture(const uint32_t& t_Color,
               const std::string& t_Name,
               bool t_IsTransparent,
               bool t_IsDoublesided)
      : Texture(t_Name, t_IsTransparent, t_IsDoublesided), m_Color{t_Color} {}

  static texture_t New(const std::string& t_Name,
